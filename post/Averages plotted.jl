...
fig = Figure(size = (width, height))
    supertitle = Label(fig[0, 1:2], "Simulated room frequency responses", fontsize = 30, tellwidth = false)
...
for (i, file) in enumerate(data_files)
    ...
    if i > 4
        ...
        ax = Axis(fig[i-4, 2],
        title = "Signal: $signal\t Cavity: sphere\t Crystal: $crystal",
        xscale = log10, xminorticksvisible = true, xminorgridvisible = true,
        xminorticks = IntervalsBetween(5), xlabel = (i == 8 ? "Frequency [Hz]" : ""))
    end
    plots = []
    ...
    if i == 8
        Legend(fig[5, 1:2], [plots;[mode]],
        [["TLLs per Wavelength: $(resolutions[j])" for j in eachindex(Fs)];["Analytical modal frequencies"]], tellwidth = false, orientation = :horizontal)
    end
end
# display(fig)
save("results/plots/averages_plotted.pdf", fig, resolution = (width, height))
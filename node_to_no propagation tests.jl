...
n, tree = Generator.nodes((10.0,10.0,10.0), crystal = "Cartesian", transmission_line_length = 1.0)
...
n_followed = 10
followed_nodes = [Int(find_nearest_node(tree, [3.5+2*i, 4.5, 4.5])) for i in 0:n_followed-1]
...
for i in 1:iterations
    ...
    Solver.outbound!(n)
    i <= 60 ? n[followed_nodes[1]].outbound[4] += sin(2*pi*timestamp) : nothing
...
ax1 = [Axis(fig[i, 1], title = "On_node_"*string(i)) for i in range1]
ax2 = [Axis(fig[i, 2], title = "On_node_"*string(i)) for i in range2.-ceil(Int64, n_followed/2)]
